...
if(isset($_POST['create-product'])){
    ...
    $stmt = $conn->prepare("INSERT INTO products (product_name,product_description,product_price,product_special_offer,
                            product_image,product_image2,product_image3,product_image4,product_category,product_color,product_size)
                            VALUES (?,?,?,?,?,?,?,?,?,?,?)");

    $stmt->bind_param('sssssssssss',$product_name,$product_description,$product_price,$product_special_offer,$image_name1,
                                    $image_name2,$image_name3,$image_name4,$product_category,$product_color,$product_size);

    if($stmt->execute()){
        header('location: products.php?product_created=Product has been Added Successfully');  
    }else{
        header('location: products.php?product_failed=Something went Wrong');
    }
